...
`;
  ...
  try {
    ...
    throw new LLMError(`Sentiment analysis failed: ${errorMessage}`, {
      originalError: error,
      modelName: config.llmModel,
      prompt: sentimentPrompt.substring(0, 200) + "...",
    });
...
`;
  try {
    ...
    if (!content) {
      ...
      throw new LLMError(
        "LLM returned empty content for message importance detection",
        {
          modelName: config.llmModel,
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error(
      "❌ [CognitiveUtils][错误] 检测重要消息LLM调用失败:",
      error instanceof BaseError ? error.toString() : errorMessage,
      error instanceof BaseError && error.details ? error.details : "",
    );
    if (error instanceof LLMError) {
    ...
    throw new LLMError(`Detecting important message failed: ${errorMessage}`, {
      originalError: error,
      modelName: config.llmModel,
      prompt: prompt.substring(0, 200) + "...",
    });
