...
* **短期记忆历史管理 (`STM_HISTORY_MODE`)**:
    * 通过 `.env` 文件中的 `STM_HISTORY_MODE` 环境变量，您可以控制爱丽丝如何管理短期记忆 (STM) 的历史记录。
    * `STM_HISTORY_MODE=kv` (默认值): 爱丽丝将在 Deno KV 存储中维护自己的 STM 历史。这是传统的行为，适用于所有交互接口。
    * `STM_HISTORY_MODE=platform` (实验性): 爱丽丝将尝试直接从连接的聊天平台 (例如 Discord) 获取近期的消息历史，用作当前交互的 STM。
        * 注意: 如果平台不支持历史记录获取 (例如，默认情况下的 CLI 接口)，或者获取尝试失败，提供给认知核心的 STM 可能会为空或非常有限。
        * 在此模式下，从平台获取的历史记录仅用于当前轮次的交互，**不会**被写回爱丽丝的 Deno KV 存储中作为 STM。
