...
本项目使用 `deno-jieba` 库进行中文分词，以提高消息处理的准确性。由于 `deno-jieba` 包含原生 Rust 代码，在首次运行项目**之前**，你需要对其进行本地编译：

1.  **安装 Rust 工具链:** 如果你的系统中没有安装 Rust 和 Cargo，请访问 [https://rustup.rs/](https://rustup.rs/) 并按照说明进行安装。
2.  **进入 `deno-jieba` 目录:** 打开你的终端或命令行工具，导航到本项目根目录下的 `deno-jieba` 文件夹：
    ```bash
    cd deno-jieba
    ```
3.  **编译库:** 运行以下命令来编译生成适用于你系统的库文件（例如 Windows 下的 `.dll`）：
    ```bash
    cargo build --release
    ```
    这将在 `deno-jieba/target/release/` 目录下生成所需的原生库文件。

完成以上步骤后，`deno-jieba` 库才能被 Deno 程序正确加载和使用。
